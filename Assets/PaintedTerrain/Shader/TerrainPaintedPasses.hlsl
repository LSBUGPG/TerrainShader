#ifndef UNIVERSAL_TERRAIN_PAINTED_PASSES
#define UNIVERSAL_TERRAIN_PAINTED_PASSES
...
half4 PaintedPixel(Texture2D tex, sampler state, half4 texelSize, half2 uv)
{
    float3 mean[4] = {
        {0, 0, 0},
        {0, 0, 0},
    ...
    for (int k = 0; k < 4; k++) {
        for (int i = 0; i <= _Radius; i++) {
            for (int j = 0; j <= _Radius; j++) {
                ...
                col = tex.SampleLevel(state, uv + float2(pos.x * texelSize.x, pos.y * texelSize.y), 0).rgb;
                mean[k] += col;
                sigma[k] += col * col;
            }
        }
...
void SplatmapMix(float4 uvMainAndLM, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    ...
    diffAlbedo[0] = PaintedPixel(_Splat0, sampler_Splat0, _Splat0_TexelSize, uvSplat01.xy);
    diffAlbedo[1] = PaintedPixel(_Splat1, sampler_Splat0, _Splat1_TexelSize, uvSplat01.zw);
    diffAlbedo[2] = PaintedPixel(_Splat2, sampler_Splat0, _Splat2_TexelSize, uvSplat23.xy);
    diffAlbedo[3] = PaintedPixel(_Splat3, sampler_Splat0, _Splat3_TexelSize, uvSplat23.zw);
